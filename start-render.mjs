...
console.log("Running OpenClaw doctor --fix before gateway startup...");
const doctor = spawnSync(
  process.execPath,
  [openclawCli, "doctor", "--fix", "--non-interactive"],
  { stdio: "inherit", env: process.env },
);
if (doctor.status !== 0) {
  console.error(`OpenClaw doctor failed with status=${doctor.status ?? "null"} signal=${doctor.signal ?? "null"}`);
